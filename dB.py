...
def dB(peak_datetimes, instrument, current_dif, jonas): #for only one instrument
    ...
    for i in range(12):
        ...
        plt.figure()
        plt.errorbar(current_dif, step_dict.get(f'Probe{num_str}_X'), yerr = step_dict.get(f'Probe{num_str}_X err'), fmt = 'b*',label = 'X') #also need to save the change in current
        X = spstats.linregress(current_dif, step_dict.get(f'Probe{num_str}_X'))
        plt.errorbar(current_dif, step_dict.get(f'Probe{num_str}_Y'), yerr = step_dict.get(f'Probe{num_str}_Y err'), fmt = 'r*', label = 'Y')
        Y = spstats.linregress(current_dif, step_dict.get(f'Probe{num_str}_Y'))
        plt.errorbar(current_dif, step_dict.get(f'Probe{num_str}_Z'), yerr = step_dict.get(f'Probe{num_str}_Z err'), fmt = 'g*', label = 'Z')
